...
(function() {
  ...
  */
  var NeuQuant, now, quality, ref, runs;
  ...
  window.addEventListener('load', function() {
    var avg, b, canvas, color, ctx, delta, el, end, g, h, i, idx, imdata, img, imgq, j, k, l, m, map, map_idx, median, o, r, ref1, ref2, ref3, ref4, ref5, results, rgb, rgb_idx, rgba, run, runtimes, start, w, x, y;
    ...
    //rgb = new Array w * h * 3
    rgb_idx = 0;
    for (i = j = 0, ref1 = rgba.length; j < ref1; i = j += 4) {
      rgb[rgb_idx++] = rgba[i + 0];
      rgb[rgb_idx++] = rgba[i + 1];
      rgb[rgb_idx++] = rgba[i + 2];
    }
    runtimes = [];
    for (run = k = 0, ref2 = runs; (0 <= ref2 ? k < ref2 : k > ref2); run = 0 <= ref2 ? ++k : --k) {
      start = now();
    ...
    avg = runtimes.reduce(function(p, n) {
      return p + n;
    }) / runtimes.length;
...
avg: ${avg.toFixed(2)}ms median: ${median.toFixed(2)}ms`);
    ...
    results = [];
    for (i = o = 0, ref5 = map.length; o < ref5; i = o += 3) {
      ...
      results.push(document.body.appendChild(el));
    }
    return results;
  });
